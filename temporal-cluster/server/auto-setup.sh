...
: "${VISIBILITY_TLS_ENABLED:=${POSTGRES_TLS_ENABLED}}"
: "${VISIBILITY_TLS_DISABLE_HOST_VERIFICATION:=${POSTGRES_TLS_DISABLE_HOST_VERIFICATION}}"
: "${VISIBILITY_TLS_CERT_FILE:=${POSTGRES_TLS_CERT_FILE}}"
: "${VISIBILITY_TLS_KEY_FILE:=${POSTGRES_TLS_KEY_FILE}}"
: "${VISIBILITY_TLS_CA_FILE:=${POSTGRES_TLS_CA_FILE}}"
: "${VISIBILITY_TLS_SERVER_NAME:=${POSTGRES_TLS_SERVER_NAME}}"
    ...
    if [[ ${ENABLE_ES} == false ]]; then
      ...
      if [[ ${VISIBILITY_DBNAME} != "${POSTGRES_USER}" && ${SKIP_DB_CREATE} != true ]]; then
          temporal-sql-tool \
              ...
              --tls="${VISIBILITY_TLS_ENABLED}" \
              --tls-disable-host-verification="${VISIBILITY_TLS_DISABLE_HOST_VERIFICATION}" \
              --tls-cert-file "${VISIBILITY_TLS_CERT_FILE}" \
              --tls-key-file "${VISIBILITY_TLS_KEY_FILE}" \
              --tls-ca-file "${VISIBILITY_TLS_CA_FILE}" \
              --tls-server-name "${VISIBILITY_TLS_SERVER_NAME}" \
              create
      fi
      temporal-sql-tool \
          --plugin ${DB} \
